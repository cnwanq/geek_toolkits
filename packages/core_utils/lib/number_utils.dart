...
static int? tryParseInt(String? value) => int.tryParse(value ?? '');

/// 安全将字符串转为 double，失败返回 null
static double? tryParseDouble(String? value) => double.tryParse(value ?? '');

/// 判断数字是否在指定范围内（包含边界）
static bool isInRange(num value, num min, num max) => value >= min && value <= max;

/// 保留小数点后 [fractionDigits] 位，返回字符串
static String toFixed(num value, int fractionDigits) =>
...
static String formatPercent(double value, {int fractionDigits = 2}) =>
    '${(value * 100).toStringAsFixed(fractionDigits)}%';

/// 补零（如 padLeftZero(5, 3) => '005'）
static String padLeftZero(int value, int width) =>
    value.toString().padLeft(width, '0');

/// 获取最大值
static T maxOf<T extends num>(List<T> list) => list.reduce((a, b) => a > b ? a : b);

/// 获取最小值
static T minOf<T extends num>(List<T> list) => list.reduce((a, b) => a < b ? a : b);
