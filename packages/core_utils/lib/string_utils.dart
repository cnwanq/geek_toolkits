...
/// 去除所有空白字符
String removeAllWhitespace(String s) => s.replaceAll(RegExp(r'\s+'), '');

/// 字符串反转
String reverse(String s) => s.split('').reversed.join();

/// 截取字符串，超出部分用省略号代替
String ellipsis(String s, int maxLength) =>
...
/// 分割字符串为 List
List<String> split(String s, String pattern) => s.split(pattern);
...
String format(String template, List<Object?> args) {
  var result = template;
